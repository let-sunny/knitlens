...
This folder contains **quality gates** and **run scripts** for KnitLens.

**Run scripts** (invoked from `.cursor/hooks.json`):

- **`append-workflow-log.js`** — Reads `.cursor/last-workflow.json`, appends a formatted entry to `WORKFLOW_LOG.md`, then deletes the JSON. Triggered on `sessionEnd` and `subagentStop`.
- **`run-typecheck-lint.js`** — Runs `pnpm run typecheck` and `pnpm run lint`. Triggered on `sessionEnd` and `subagentStop` so that type and lint are checked at the end of a session or subagent run. The hooks are designed as a **scaffold**, not a final implementation. They encode the project rules you already defined:
